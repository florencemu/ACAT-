...
class Subject extends CI_Controller {
	...
	public function sub_add(){
		...
		$sub_que = $this->input->get('sub_que');
		$sub_ans = $this->input->get('sub_ans');
		...
		$sub_diff = $this->input->get('sub_diff');
	...
	public function sub_mod(){
		...
		$this->load->model('subjects','subjects');
		$s_id = $this->input->get('id');
		$que  = $this->input->get('question');
		$ans = $this->input->get('answer');
		$diff  = $this->input->get('sub_diff');
		if(!$diff) error("请选择试题难度！");
		$type = $this->input->get('sub_type');
		switch ($type) {
					case '1': $type= '基础题';break;
					case '2': $type= 'PHP';break;
					case '3': $type= '前端';break;
